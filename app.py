...
def _dw_trade_browser(res):
    ...
    if dd.empty:
        st.info("No minute data for this trade's day.")
        return
    vwap = _session_vwap_day(dd) if show_vwap else None           # session VWAP overlay
    ...
    st.caption("Shaded box = IB window · grey dotted = entry level · red dashed = stop · "
               ...
               "amber line = session VWAP (exit uses 5-min VWAP) · ▲/▼ entry · ✕ final exit.")
...
@st.cache_data(show_spinner=False)
def prep_cpr(instrument, mpath, mtime, open_t, close_t, ib_min):
    """Per-day CPR records on 5-min bars (CPR from the prior session's H/L/C)."""
    m5 = build_facts.to_timeframe(load_min(mpath, mtime), EXEC_TF, open_t)
    return cpr.prep_days(m5, open_t, close_t, ib_min, tf=EXEC_TF)


CPR_LOG_COLS = cpr.LOG_COLS


def _cpr_day_chart(dd, tr, open_t, ib_min):
    """One CPR trade on its day's 5-min chart — full CPR + floor pivots + IB box."""
    day = pd.Timestamp(tr["date"])
    x = dd["date"]
    ...
    # central pivot range (TC..BC) + pivot, then the floor pivots
    fig.add_shape(type="rect", x0=x0, x1=x1, y0=tr["bc"], y1=tr["tc"],
                  fillcolor="rgba(21,101,192,0.08)", line=dict(width=0), layer="below")

    def _lvl(y, color, dash, width, label, lbl=True):
        if pd.isna(y):
            return
        fig.add_shape(type="line", x0=x0, x1=x1, y0=y, y1=y,
                      line=dict(color=color, width=width, dash=dash))
        if lbl:
            fig.add_annotation(x=x1, y=y, text=label, showarrow=False, xanchor="left",
                               font=dict(size=9, color=color),
                               bgcolor="rgba(255,255,255,0.65)")

    _lvl(tr["tc"], "#1565C0", "solid", 1.1, "TC")
    _lvl(tr["pp"], "#1565C0", "dash", 1.5, "P")
    _lvl(tr["bc"], "#1565C0", "solid", 1.1, "BC")
    _lvl(tr["r1"], "#E53935", "dot", 1.1, "R1")
    _lvl(tr["s1"], "#2E7D32", "dot", 1.1, "S1")
    _lvl(tr["r2"], "#EF9A9A", "dot", 0.8, "R2")
    _lvl(tr["s2"], "#A5D6A7", "dot", 0.8, "S2")
    _lvl(tr["r3"], "#EF9A9A", "dot", 0.6, "R3", lbl=False)
    _lvl(tr["s3"], "#A5D6A7", "dot", 0.6, "S3", lbl=False)

    # initial-balance box
    if not pd.isna(tr["ib_high"]):
        fig.add_shape(type="rect",
                      x0=day + pd.Timedelta(minutes=open_t),
                      x1=day + pd.Timedelta(minutes=open_t + ib_min - 1),
                      y0=tr["ib_low"], y1=tr["ib_high"],
                      fillcolor="rgba(96,125,139,0.06)", line=dict(width=1, color="#B0BEC5"))

    # trade overlay
    x_e = day + pd.Timedelta(minutes=int(tr["entry_t"]))
    x_x = day + pd.Timedelta(minutes=int(tr["exit_t"]))
    fig.add_shape(type="line", x0=x_e, x1=x_x, y0=tr["entry"], y1=tr["entry"],
                  line=dict(width=1.1, color="#455A64", dash="dot"))
    fig.add_shape(type="line", x0=x_e, x1=x_x, y0=tr["stop"], y1=tr["stop"],
                  line=dict(width=1.4, color="#C62828", dash="dash"))
    if not pd.isna(tr["tp1"]):
        fig.add_shape(type="line", x0=x_e, x1=x_x, y0=tr["tp1"], y1=tr["tp1"],
                      line=dict(width=1.1, color="#66BB6A", dash="dot"))
    fig.add_shape(type="line", x0=x_e, x1=x_x, y0=tr["tp2"], y1=tr["tp2"],
                  line=dict(width=1.4, color="#2E7D32", dash="dash"))
    long = tr["direction"] == "long"
    win_c = "#2E7D32" if tr["win"] else "#C62828"
    fig.add_scatter(x=[x_e], y=[tr["entry"]], mode="markers",
                    marker=dict(symbol="triangle-up" if long else "triangle-down", size=13,
                                color="#2E7D32" if long else "#C62828",
                                line=dict(width=1, color="white")), showlegend=False,
                    hovertext=f"entry {hhmm(tr['entry_t'])} · {tr['entry']:.2f}", hoverinfo="text")
    fig.add_scatter(x=[x_x], y=[tr["exit"]], mode="markers",
                    marker=dict(symbol="x", size=11, color=win_c), showlegend=False,
                    hovertext=f"exit {hhmm(tr['exit_t'])} · {tr['exit']:.2f} ({tr['outcome']})",
                    hoverinfo="text")
    fig.add_scatter(x=[x_e, x_x], y=[tr["entry"], tr["exit"]], mode="lines",
                    line=dict(width=1, color=win_c, dash="dot"), showlegend=False, hoverinfo="skip")
    _clean_layout(fig, height=560,
                  title=f"{day:%a %d %b %Y} · {cpr.SYSTEM_LABELS.get(tr['system'], tr['system'])} · "
                        f"{tr['direction'].upper()} · {tr['outcome']} · {tr['r']:+.2f}R · "
                        f"{tr['pnl']:+.1f} pts · {tr['relationship']} ({tr['width_pct']:.2f}% CPR)")
    st.plotly_chart(fig, use_container_width=True, config={"scrollZoom": True})


def _cpr_trade_browser(trades, mpath, mtime, open_t, close_t, ib_min, key="cpr"):
    t_ = trades.reset_index(drop=True)
    labels = [f"#{i+1} · {r['date']:%d %b %Y} {hhmm(r['entry_t'])} · "
              f"{cpr.SYSTEM_LABELS.get(r['system'], r['system']).split('·')[0].strip()} · "
              f"{r['direction'].upper()} · {r['outcome']} · {r['r']:+.2f}R ({r['pnl']:+.1f} pts)"
              for i, r in t_.iterrows()]
    sel = st.selectbox("Trade", range(len(t_)), format_func=lambda i: labels[i], key=f"{key}_tb")
    tr = t_.iloc[sel]
    mdf = load_min(mpath, mtime)
    dd = mdf[(mdf["date_only"] == pd.Timestamp(tr["date"]).date())
             & (mdf["t_min"] >= open_t) & (mdf["t_min"] <= close_t)]
    ...
    _cpr_day_chart(dd, tr, open_t, ib_min)
    st.caption("5-min candles · blue band = CPR (TC–BC), dashed = Pivot · red dotted = "
               "R1/R2/R3 · green dotted = S1/S2/S3 · grey box = Initial Balance · "
               "grey dotted = entry · red dashed = stop · green dotted = TP1 · "
               "green dashed = TP2 · ▲/▼ entry · ✕ exit.")


# ── CPR confluence (combine systems / configs into one portfolio) ──────────────
CPR_CONFL_FILE = os.path.join(HERE, "analysis", "cpr_confluence_presets.json")


def _cpr_confl_load():
    if os.path.exists(CPR_CONFL_FILE):
        try:
            return json.load(open(CPR_CONFL_FILE, encoding="utf-8"))
        except Exception:
            pass
    return {"presets": {}, "last": []}


def _cpr_confl_save(data):
    os.makedirs(os.path.dirname(CPR_CONFL_FILE), exist_ok=True)
    json.dump(data, open(CPR_CONFL_FILE, "w", encoding="utf-8"), indent=2)


def _cpr_confl_components():
    if "cpr_confl" not in st.session_state:
        st.session_state["cpr_confl"] = _cpr_confl_load().get("last") or []
    return st.session_state["cpr_confl"]


def _cpr_confl_persist():
    store = _cpr_confl_load()
    store["last"] = st.session_state.get("cpr_confl", [])
    _cpr_confl_save(store)


def _cpr_confl_label(c):
    cfg = c["cfg"]
    extra = ""
    if cfg["system"] == "trend":
        extra = f" · mode {cfg['trend_mode']}"
    return (f"{cpr.SYSTEM_LABELS.get(cfg['system'], cfg['system'])}{extra} · "
            f"wick {cfg['wick_mult']}× · loc {cfg['close_loc']} · "
            f"scale {'on' if cfg['scale_out'] else 'off'} · {c['instrument']}")


def _cpr_metric_row(m):
    k = st.columns(6)
    k[0].metric("Days", f"{m['n_days']:,}")
    k[1].metric("Trades", f"{m['trades']:,}")
    k[2].metric("Win rate", pct(m["win_rate"]))
    ...
    k2 = st.columns(6)
    k2[0].metric("Win / Loss", f"{m['wins']} / {m['losses']}")
    k2[1].metric("Expectancy", f"{m['expectancy']:.2f} pts")
    k2[2].metric("Avg R", f"{m['avg_r']:+.2f}R")
    k2[3].metric("Max DD", f"{m['max_dd']:,.0f} pts")
    k2[4].metric("Avg Win", f"{m['avg_win']:.1f} pts")
    k2[5].metric("Sharpe", f"{m['sharpe']:.2f}" if np.isfinite(m["sharpe"]) else "—")


def _cpr_log_df(trades):
    log = trades.copy()
    log["entry time"] = log["entry_t"].map(hhmm)
    log["exit time"] = log["exit_t"].map(hhmm)
    return log
...
def _cpr_controls(open_t, close_t, ib_min):
    """All CPR conditions (triggers · stop/targets · bias & regime · timing).
    Returns a fully-specified cfg dict for the engine."""
    cfg = cpr.default_cfg(open_t, close_t, ib_min)

    sysname = st.radio("System", cpr.SYSTEMS, horizontal=True, key="cpr_system",
                       format_func=lambda s: cpr.SYSTEM_LABELS[s])
    cfg["system"] = sysname

    # ── per-system bias / mode controls ───────────────────────────────────────
    if sysname == "reversal":
        st.markdown("**Bias filter (two-day CPR relationship must agree)**")
        c = st.columns(2)
        cfg["rev_short_rel"] = c[0].multiselect(
            "Short setups — allowed relationships", cpr.RELATIONSHIPS,
            default=cfg["rev_short_rel"], key="cpr_revshort",
            help="Book's 65% fade: open below CPR, rally into it, reject down.")
        cfg["rev_long_rel"] = c[1].multiselect(
            "Long setups — allowed relationships", cpr.RELATIONSHIPS,
            default=cfg["rev_long_rel"], key="cpr_revlong",
            help="Off by default: the 59% long fade tested as a net drag on NIFTY. "
                 "Add relationships to enable longs (use the stronger trigger below).")
        cc = st.columns(2)
        cfg["strong_long"] = cc[0].checkbox("Require stronger trigger on longs",
                                            value=True, key="cpr_strong",
                                            help="Offsets the lower long base rate (book asymmetry).")
        cfg["skip_trap"] = cc[1].checkbox("Skip Inside-Value breakout trap days",
                                          value=True, key="cpr_skiptrap")
    elif sysname == "trend":
        cm = st.columns([1.2, 1, 1])
        cfg["trend_mode"] = cm[0].selectbox(
            "Mode", ["A", "B", "both"], index=2, key="cpr_tmode",
            format_func=lambda m: {"A": "A · pull-back", "B": "B · breakout",
                                   "both": "Both (A first, else B)"}[m])
        cfg["use_breakout"] = cm[1].checkbox("Enable Mode B", value=True, key="cpr_useb")
        cfg["atr_mult"] = cm[2].number_input("Trail × ATR (Mode B)", 0.5, 4.0, 1.5, 0.25,
                                             key="cpr_atr")
        c = st.columns(2)
        cfg["trend_up_rel"] = c[0].multiselect("Uptrend relationships", cpr.RELATIONSHIPS,
                                               default=cfg["trend_up_rel"], key="cpr_tup")
        cfg["trend_dn_rel"] = c[1].multiselect("Downtrend relationships", cpr.RELATIONSHIPS,
                                               default=cfg["trend_dn_rel"], key="cpr_tdn")
        cfg["breakout_rel"] = st.multiselect("Mode B breakout relationships", cpr.RELATIONSHIPS,
                                             default=cfg["breakout_rel"], key="cpr_brel",
                                             help="Open beyond yesterday's range + one of these.")
    else:  # scalp
        c = st.columns([1.4, 1, 1])
        cfg["scalp_rel"] = c[0].multiselect("Range-regime relationships", cpr.RELATIONSHIPS,
                                            default=cfg["scalp_rel"], key="cpr_srel",
                                            help="Scalp also fires whenever the CPR is 'wide'.")
        cfg["scalp_fence"] = c[1].selectbox("Fences", ["S1/R1", "CPR edges"], key="cpr_fence")
        c[2].caption("Kill switch (stop scalping after a decisive close beyond a fence) "
                     "is always on.")
        cfg["scalp_block_breakout"] = st.checkbox(
            "Stand aside on opens beyond yesterday's range", value=True, key="cpr_sblock")

    # ── re-entry (frequency lever — applies to all systems) ────────────────────
    rc = st.columns([1.3, 1, 2.2])
    cfg["allow_reentry"] = rc[0].checkbox("Allow re-entry (more trades/day)", value=True,
                                          key="cpr_reentry",
                                          help="Take a 2nd/3rd non-overlapping trade per day "
                                               "after one closes. Main lever for hitting "
                                               "3–4 trades/week.")
    cfg["max_per_day"] = int(rc[1].number_input("Max trades/day", 1, 6, 3, 1,
                                                key="cpr_maxday",
                                                disabled=not cfg["allow_reentry"]))
    rc[2].caption("Frequency tip: ~3–4 trades/week needs the loosened trigger (Wick ~2.0, "
                  "both directions, all relationships) **plus** re-entry, run on **both** "
                  "NIFTY 50 and BANK NIFTY. Strict defaults give the highest win rate but "
                  "only ~0.8/week.")

    # ── triggers ──────────────────────────────────────────────────────────────
    with st.expander("🎯 Entry triggers (Wick / Doji reversal)", expanded=True):
        t = st.columns(4)
        cfg["wick_mult"] = t[0].number_input("Wick ≥ × body", 1.0, 5.0,
                                             float(cfg["wick_mult"]), 0.25, key="cpr_wick")
        cfg["close_loc"] = t[1].number_input("Close location (outer)", 0.5, 0.95,
                                             float(cfg["close_loc"]), 0.05, key="cpr_loc",
                                             help="0.65 = close in the outer 35% away from the level.")
        cfg["doji_on"] = t[2].checkbox("Doji also triggers", value=True, key="cpr_doji")
        cfg["doji_frac"] = t[3].number_input("Doji body ≤ × range", 0.02, 0.30,
                                             float(cfg["doji_frac"]), 0.02, key="cpr_dojifrac")
        t2 = st.columns(2)
        cfg["touch_pct"] = t2[0].number_input("Level touch tolerance (% of price)", 0.0, 0.5,
                                              float(cfg["touch_pct"]), 0.01, key="cpr_touch")
        cfg["buf_pct"] = t2[1].number_input("Stop buffer beyond candle (% of price)", 0.0, 0.5,
                                            float(cfg["buf_pct"]), 0.01, key="cpr_buf")

    # ── stop / targets ────────────────────────────────────────────────────────
    with st.expander("🛡️ Stop · Targets · scale-out", expanded=True):
        s = st.columns(3)
        cfg["scale_out"] = s[0].checkbox("Scale out 50% at TP1 (Pivot)", value=True,
                                         key="cpr_scale",
                                         help="Book half at the Pivot; runner to TP2.")
        cfg["be_after_tp1"] = s[1].checkbox("Move stop to breakeven after TP1",
                                            value=True, key="cpr_be")
        s[2].caption("TP1 = Pivot · TP2 = outer floor pivot, modulated by CPR width "
                     "(narrow → run to S2/R2 · wide → near edge only).")
        w = st.columns(2)
        cfg["narrow_pct"] = w[0].number_input("Narrow CPR < (% of price)", 0.02, 1.0,
                                              float(cfg["narrow_pct"]), 0.01, key="cpr_narrow")
        cfg["wide_pct"] = w[1].number_input("Wide CPR > (% of price)", 0.05, 2.0,
                                            float(cfg["wide_pct"]), 0.01, key="cpr_wide")

    # ── timing ────────────────────────────────────────────────────────────────
    with st.expander("⏱️ Timing", expanded=False):
        tt = st.columns(3)
        es = tt[0].time_input("Entries from", value=dtime(cfg["entry_start_t"] // 60,
                                                          cfg["entry_start_t"] % 60), key="cpr_es")
        ee = tt[1].time_input("Entries until", value=dtime(cfg["entry_end_t"] // 60,
                                                          cfg["entry_end_t"] % 60), key="cpr_ee")
        ss = tt[2].time_input("Scalp starts after IB", value=dtime(cfg["scalp_start_t"] // 60,
                                                                   cfg["scalp_start_t"] % 60),
                              key="cpr_ss")
        cfg["entry_start_t"] = es.hour * 60 + es.minute
        cfg["entry_end_t"] = ee.hour * 60 + ee.minute
        cfg["scalp_start_t"] = ss.hour * 60 + ss.minute
        st.caption(f"Square-off (force exit) is the sidebar's setting: "
                   f"**{hhmm(close_t)}** — nothing carries overnight.")
    cfg["eff_min"] = close_t
    return cfg
...
def cpr_mode():
    st.title("📈 CPR Algos")
    st.caption("Central Pivot Range systems (Frank Ochoa · *Secrets of a Pivot Boss*), "
               "calibrated for NIFTY. Three systems — Reversal · Trend · Scalp — on 5-min "
               "execution, with the two-day CPR relationship as the bias filter, Wick/Doji "
               "triggers, and scale-out targets. Optimizer · Confluence · trade browser.")
    with st.sidebar:
        instrument, mpath, mtime, open_t, close_t, ib_min, (d0, d1) = data_sidebar("cpr")
    if instrument not in ("NIFTY 50", "BANK NIFTY"):
        st.info(f"CPR thresholds are NIFTY-calibrated (% of price). Running on **{instrument}** "
                "is exploratory — re-tune narrow/wide width and tolerances for it.")

    with st.spinner(f"Indexing {instrument} CPR (first run cached)…"):
        prep_all = prep_cpr(instrument, mpath, mtime, open_t, close_t, ib_min)
    prep = [r for r in prep_all if d0 <= r["date"].date() <= d1]
    if not prep:
        st.warning("No trading days in the selected date range.")
        st.stop()

    cfg = _cpr_controls(open_t, close_t, ib_min)
    st.divider()
    tab1, tab2, tab3 = st.tabs(["🎯 Strategy", "🔬 Optimizer", "🧩 Confluence"])

    # ── strategy ──────────────────────────────────────────────────────────────
    with tab1:
        trades = cpr.run(prep, cfg)
        m = cpr.metrics(trades, len(prep))
        if m is None:
            st.warning("No trades for this configuration. Loosen the triggers / bias filters, "
            ...
            eq = cpr.equity_curve(trades, prep)
            fig = go.Figure()
            fig.add_scatter(x=eq["date"], y=eq["equity"], mode="lines",
                            line=dict(color="#2196F3", width=2), fill="tozeroy",
                            fillcolor="rgba(33,150,243,0.10)")
            fig.update_layout(title="Equity curve (cumulative points)", height=320,
                              template="plotly_white", margin=dict(t=40, b=10),
                              yaxis_title="pts")
            st.plotly_chart(fig, use_container_width=True)

            if st.button("➕ Add this configuration to Confluence", key="cpr_add"):
                comps = _cpr_confl_components()
                comp = {"instrument": instrument, "cfg": copy.deepcopy(cfg)}
                if comp not in comps:
                    comps.append(comp)
                    _cpr_confl_persist()
                    st.success("Added to the Confluence tab.")
                else:
                    st.info("Already in Confluence.")

            log = _cpr_log_df(trades)
            with st.expander(f"📋 All trades ({len(log):,}) — entry & exit times"):
                st.dataframe(log[CPR_LOG_COLS].sort_values("date", ascending=False),
                             use_container_width=True, hide_index=True, height=380)
                st.download_button("⬇ Download trades (CSV)",
                                   log[CPR_LOG_COLS].to_csv(index=False).encode(),
                                   file_name=f"cpr_{cfg['system']}_{instrument.replace(' ', '_')}.csv",
                                   mime="text/csv", key="cpr_dl")
            with st.expander("🔍 Trade browser — any trade on its day's chart", expanded=True):
                _cpr_trade_browser(trades, mpath, mtime, open_t, close_t, ib_min)

    # ── optimizer ─────────────────────────────────────────────────────────────
    with tab2:
        st.markdown("Sweep the trigger & geometry knobs for the **selected system** "
                    "(everything else stays as set on the Strategy tab). Ranked, with a "
                    "minimum-trades guard.")
        o = st.columns(3)
        rank = o[0].selectbox("Rank by", ["net_r", "net", "win_rate", "pf", "expectancy"],
                              format_func=lambda x: {"net_r": "Net R", "net": "Net pts",
                                                     "win_rate": "Win %", "pf": "Profit factor",
                                                     "expectancy": "Expectancy"}[x], key="cpr_rank")
        min_tr = int(o[1].number_input("Min trades", 10, 5000, 40, 10, key="cpr_min"))
        o[2].caption(f"Optimizing **{cpr.SYSTEM_LABELS[cfg['system']]}**.")
        g = st.columns(3)
        wg = g[0].multiselect("Wick × grid", [1.5, 2.0, 2.5, 3.0, 3.5],
                              default=[2.0, 2.5, 3.0], key="cpr_wg")
        lg = g[1].multiselect("Close-loc grid", [0.55, 0.6, 0.65, 0.7, 0.75, 0.85],
                              default=[0.6, 0.65, 0.75], key="cpr_lg")
        bg = g[2].multiselect("Stop buffer % grid", [0.04, 0.08, 0.12, 0.16, 0.20, 0.30],
                              default=[0.08, 0.12, 0.16, 0.20], key="cpr_bg")
        sw = st.columns(2)
        so = sw[0].multiselect("Scale-out", ["on", "off"], default=["on", "off"], key="cpr_so")
        be = sw[1].multiselect("Breakeven after TP1", ["on", "off"], default=["on"], key="cpr_beg")

        if wg and lg and bg and so and be:
            n_combo = len(wg) * len(lg) * len(bg) * len(so) * len(be)
            tone = "⚠️ " if n_combo > 200 else ""
            st.caption(f"{tone}**{n_combo:,} configurations** × {len(prep):,} days each. "
                       "Narrow the date range for big sweeps.")

        if st.button("🔎 Run optimization", type="primary", key="cpr_opt"):
            if not (wg and lg and bg and so and be):
                st.warning("Pick at least one value in every grid.")
            else:
                grids = dict(wick_mult=sorted(wg), close_loc=sorted(lg), buf_pct=sorted(bg),
                             scale_out=[v == "on" for v in so], be=[v == "on" for v in be])
                prog = st.progress(0.0, text="Sweeping…")
                ...
                res = cpr.optimize(prep, cfg, grids, min_tr, rank, progress=cb)
                prog.empty()
                st.session_state["cpr_opt_res"] = res

        res = st.session_state.get("cpr_opt_res")
        if res is not None:
            if res.empty:
                st.warning("No configuration met the minimum-trades threshold.")
            else:
                ...
                st.dataframe(res.head(40), use_container_width=True, hide_index=True)
                st.download_button("⬇ Download results", res.to_csv(index=False).encode(),
                                   file_name=f"cpr_optimizer_{cfg['system']}.csv",
                                   mime="text/csv", key="cpr_optdl")

    # ── confluence ────────────────────────────────────────────────────────────
    with tab3:
        _cpr_confluence_tab(prep, instrument, mpath, mtime, open_t, close_t, ib_min)


def _cpr_confluence_tab(prep, instrument, mpath, mtime, open_t, close_t, ib_min):
    st.markdown("Combine several CPR configurations (e.g. Reversal + Trend + Scalp) into one "
                "portfolio — uncorrelated systems smooth the equity curve. Add components with "
                "the button on the **Strategy** tab.")
    comps = _cpr_confl_components()

    store = _cpr_confl_load()
    with st.expander("💾 Save / Load confluences", expanded=False):
        s_ = st.columns([2, 1, 2, 1, 1])
        pname = s_[0].text_input("Confluence name", key="cpr_cpname", placeholder="e.g. cpr-core")
        if s_[1].button("Save", key="cpr_csave", use_container_width=True):
            if not comps:
                st.warning("Nothing to save — add configurations first.")
            elif pname.strip():
                store["presets"][pname.strip()] = comps
                store["last"] = comps
                _cpr_confl_save(store)
                ...
                st.warning("Enter a name first.")
        names = list(store.get("presets", {}).keys())
        sel = s_[2].selectbox("Saved confluences", ["—"] + names, key="cpr_csel")
        if s_[3].button("Load", key="cpr_cload", use_container_width=True) and sel != "—":
            st.session_state["cpr_confl"] = list(store["presets"][sel])
            _cpr_confl_persist()
            st.rerun()
        if s_[4].button("Delete", key="cpr_cdel", use_container_width=True) and sel != "—":
            store["presets"].pop(sel, None)
            _cpr_confl_save(store)
            st.rerun()

    if not comps:
        st.info("No configurations added yet. Build one on the Strategy tab and click "
                "“➕ Add this configuration to Confluence”.")
        return

    list_df = pd.DataFrame({"#": range(1, len(comps) + 1),
                            "configuration": [_cpr_confl_label(c) for c in comps],
                            "instrument": [c["instrument"] for c in comps]})
    st.dataframe(list_df, use_container_width=True, hide_index=True)
    cdel = st.columns([2, 1, 1])
    rm = cdel[0].multiselect("Remove #", list(range(1, len(comps) + 1)), key="cpr_crm")
    if cdel[1].button("Remove selected", use_container_width=True, key="cpr_crmbtn") and rm:
        st.session_state["cpr_confl"] = [c for i, c in enumerate(comps, 1) if i not in rm]
        _cpr_confl_persist()
        st.rerun()
    if cdel[2].button("Clear all", use_container_width=True, key="cpr_cclr"):
        st.session_state["cpr_confl"] = []
        _cpr_confl_persist()
        st.rerun()

    cur = [c for c in comps if c["instrument"] == instrument]
    if len(cur) < len(comps):
        st.caption(f"⚠ {len(comps) - len(cur)} component(s) belong to a different instrument "
                   "and are excluded here — switch the sidebar instrument to run them.")
    if not cur:
        st.warning(f"No components for {instrument}.")
        return

    frames = []
    for i, c in enumerate(cur, 1):
        tr = cpr.run(prep, c["cfg"])
        if tr.empty:
            continue
        tr["strategy"] = f"#{i}"
        frames.append(tr)
    if not frames:
        ...
        return
    T = pd.concat(frames).sort_values(["date", "entry_t"]).reset_index(drop=True)
    m = cpr.metrics(T, len(prep))
    _cpr_metric_row(m)

    cum = np.cumsum(T["pnl"].to_numpy())
    dd = cum - np.maximum.accumulate(cum)
    dts = pd.to_datetime(T["date"])
    fig = go.Figure()
    fig.add_scatter(x=dts, y=cum, mode="lines", name="Portfolio equity",
                    line=dict(color="#2196F3", width=2), fill="tozeroy",
                    fillcolor="rgba(33,150,243,0.08)")
    fig.update_layout(title="Combined equity (points)", height=300, template="plotly_white",
                      margin=dict(t=40, b=10))
    st.plotly_chart(fig, use_container_width=True)
    figd = go.Figure()
    figd.add_scatter(x=dts, y=dd, mode="lines", line=dict(color="#E53935", width=1.5),
                     fill="tozeroy", fillcolor="rgba(229,57,53,0.15)")
    figd.update_layout(title="Drawdown (points)", height=200, template="plotly_white",
                       margin=dict(t=40, b=10))
    st.plotly_chart(figd, use_container_width=True)

    st.markdown("**Per-configuration contribution**")
    contrib = (T.groupby("strategy")
               .agg(trades=("pnl", "size"), net=("pnl", "sum"),
                    win_rate=("win", "mean"), expectancy=("pnl", "mean")))
    contrib["win_rate"] = (contrib["win_rate"] * 100).round(1)
    ...
    log = _cpr_log_df(T)
    with st.expander(f"📋 All portfolio trades ({len(T):,})"):
        st.dataframe(log[["strategy"] + CPR_LOG_COLS].sort_values("date", ascending=False),
                     use_container_width=True, hide_index=True, height=360)
    st.download_button("⬇ Download portfolio trades (CSV)",
                       log[["strategy"] + CPR_LOG_COLS].to_csv(index=False).encode(),
                       file_name=f"cpr_confluence_{instrument.replace(' ', '_')}.csv",
                       mime="text/csv", key="cpr_confldl")
    with st.expander("🔍 Trade browser — any portfolio trade on its day's chart"):
        _cpr_trade_browser(T, mpath, mtime, open_t, close_t, ib_min, key="cprconfl")
...
def main():
    ...
    mode = st.sidebar.radio(
        "Mode",
        ["Edge Backtester", "CPR Algos", "Day-wise IB Retracement", "IB50",
